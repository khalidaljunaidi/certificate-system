...
export default async function AdminLoginPage() {
  ...
  return (
    <main className="relative isolate min-h-screen overflow-hidden bg-[#05060d] text-white">
      ...
      <div
        className="pointer-events-none absolute left-1/2 top-1/2 h-[700px] w-[700px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/12"
        style={{
          background:
            "conic-gradient(from 0deg,rgba(255,255,255,0.14),rgba(255,255,255,0),rgba(181,128,255,0.18),rgba(255,255,255,0))",
          animation: "lux-orbit 68s linear infinite",
        }}
      />
      ...
      <div
        className="tg-surface-live pointer-events-none absolute right-[-10%] top-[38%] h-[520px] w-[520px] rounded-full bg-[radial-gradient(circle,rgba(116,61,163,0.25)_0%,rgba(116,61,163,0.05)_52%,rgba(116,61,163,0)_78%)] blur-3xl"
        style={{ animation: "drift-y 26s ease-in-out infinite" }}
      />
      <div className="pointer-events-none absolute inset-x-[-20%] top-[35%] h-[16px] border-t border-white/30" />
      <div
        className="pointer-events-none absolute inset-y-[-30%] left-[16%] h-72 w-[220px] rotate-12 rounded-full border border-white/20 bg-[linear-gradient(90deg,rgba(255,255,255,0)_0%,rgba(255,255,255,0.26)_48%,rgba(255,255,255,0)_100%)] blur-sm"
        style={{ animation: "sweep 20s linear infinite" }}
      />
      <div
        className="pointer-events-none absolute inset-y-[72%] left-[32%] h-64 w-[240px] -rotate-12 rounded-full border border-white/16 bg-[linear-gradient(90deg,rgba(255,255,255,0)_0%,rgba(255,255,255,0.16)_48%,rgba(255,255,255,0)_100%)] blur-sm"
        style={{ animation: "sweep 28s linear infinite" }}
      />
      <div
        className="pointer-events-none absolute inset-x-0 top-10 mx-auto h-full w-full bg-[linear-gradient(100deg,transparent_0,rgba(255,255,255,0.05)_50%,transparent_100%)] bg-[length:220%_220%]"
        style={{ animation: "lux-ribbon 36s ease-in-out infinite" }}
      />
      <div
        className="pointer-events-none absolute inset-x-0 top-10 mx-auto h-full w-full bg-[linear-gradient(100deg,transparent_0,rgba(255,255,255,0.06)_50%,transparent_100%)] bg-[length:220%_220%]"
        style={{ animation: "drift-x 30s ease-in-out infinite" }}
      />
      <div
        className="pointer-events-none absolute left-[24%] top-[18%] h-1.5 w-1.5 rounded-full bg-white/65 shadow-[0_0_18px_rgba(255,255,255,0.66)]"
        style={{ animation: "lux-float-a 17s ease-in-out infinite" }}
      />
      <div
        className="pointer-events-none absolute left-[74%] top-[36%] h-2.5 w-2.5 rounded-full bg-[#f8caa0] shadow-[0_0_22px_rgba(248,202,160,0.55)]"
        style={{ animation: "lux-float-b 19s ease-in-out infinite" }}
      />
      <div
        className="pointer-events-none absolute left-[14%] top-[62%] h-2 w-2 rounded-full bg-[#9d79ff] shadow-[0_0_20px_rgba(157,121,255,0.52)]"
        style={{ animation: "lux-float-c 21s ease-in-out infinite" }}
      />
        ...
        <div className="grid w-full items-stretch gap-6 lg:grid-cols-[3fr_2fr]">
            ...
            <div className="relative z-10 flex h-full flex-col justify-between">
              <div>
                ...
                <p className="mt-8 max-w-xl text-base leading-7 text-white/76">
                  Enterprise governance for suppliers, vendors, tasks, projects,
          ...
          <section className="flex items-center justify-center">
            <div className="w-full max-w-md rounded-[28px] border border-white/25 bg-white/92 p-8 text-[var(--color-ink)] shadow-[0_28px_100px_rgba(7,9,22,0.28)] backdrop-blur-xl">
              <p className="text-[11px] font-medium uppercase tracking-[0.10em] text-[var(--color-primary)]">
                Internal System
              </p>
              ...
              <p className="mt-3 text-sm leading-7 text-[var(--color-muted)]">
                Sign in to The Gathering KSA Procurement Operations Platform.
              </p>
